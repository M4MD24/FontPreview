...
def generate_high_resolution_font_preview(
    ...
    """
    Generate a high-resolution grid preview of all fonts in a list or folder.
    Each font gets a frame with sample texts (Arabic, Arabic-Indic digits,
    European digits, English) and the font name.
    """
    if columns <= 0:
    ...
    if sample_text is None:
        sample_text = [
            {"text": "اللغة العربية", "is_rtl": True},
            {"text": "٠١٢٣٤٥٦٧٨٩", "is_rtl": True},
            {"text": "0123456789", "is_rtl": True},
            {"text": "English Language", "is_rtl": False}
        ]

    # Validate sample_text structure
    for item in sample_text:
        if not isinstance(item, dict):
            raise TypeError("Each sample_text item must be a dictionary with 'text' and 'is_rtl' keys.")
        if "text" not in item or "is_rtl" not in item:
            raise ValueError("Each sample_text item must contain 'text' and 'is_rtl'.")
    ...
    if not use_raqm_effective:
        print("Warning: RAQM is disabled or unavailable. Arabic shaping and RTL layout may not work correctly.")

    # Collect font files if not provided directly
    if font_files is None:
        ...
        for font_file in font_files:
            key = os.path.normcase(os.path.abspath(font_file))
    ...
    # Effective columns: min(columns, number of fonts)
    effective_columns = min(columns, len(font_files))

    # Load title font if provided
    if title_font_path and os.path.isfile(title_font_path):
        try:
            ...
            print(f"Warning: could not load title font '{title_font_path}': {error}")
            title_font = ImageFont.load_default()
    else:
        title_font = None

    rows = math.ceil(len(font_files) / effective_columns)
    image_width = effective_columns * actual_cell_width + actual_padding * 2
    image_height = rows * actual_cell_height + actual_padding * 2

    print(f"Grid: {rows}x{effective_columns} (requested columns: {columns}) | Image: {image_width}x{image_height}")
    ...
    for index, font_path in enumerate(font_files):
        ...
        x = actual_padding + column * actual_cell_width
        ...
        # Draw cell border
        draw.rectangle(
            [x, y, x + actual_cell_width, y + actual_cell_height],
            outline=grid_color,
            width=max(2, int(2 * scale))
        )

        try:
            # Load the font with or without RAQM
            if use_raqm_effective:
                font = ImageFont.truetype(font_path, actual_font_size, layout_engine=ImageFont.Layout.RAQM)
            else:
                font = ImageFont.truetype(font_path, actual_font_size)

            # Load name font (cache)
            ...
            # Draw font name centered in the upper part
            name_center_x = x + actual_cell_width / 2
            name_center_y = y + actual_cell_height / 4 + actual_title_vertical_offset
            draw.text((name_center_x, name_center_y), font_name, font=name_font, fill=title_color, anchor="mm")

            # Draw preview texts centered in the lower part
            if sample_text:
                ...
                total_height = sum(line_heights) + (len(sample_text) - 1) * actual_line_spacing
                ...
                for idx, item in enumerate(sample_text):
                    ...
                    line_center_x = x + actual_cell_width / 2

                    if use_raqm_effective:
                        direction = "rtl" if is_rtl else "ltr"
                        draw.text((line_center_x, line_center_y), text, font=font,
            ...
            print(f"Error rendering {font_path}: {error}")
...
def generate_previews_for_subfolders(
    ...
    """
    Walk through root_folder and generate a preview image for each subfolder
    that contains font files. If combine_all is True, generate a single preview
    image containing all fonts from all subfolders combined.
    """
    ...
    if combine_all:
        # Collect all font files from all subfolders recursively
        all_font_files = []
        for root, dirs, files in os.walk(root_folder):
            for file_name in files:
                if file_name.lower().endswith((".ttf", ".otf")):
                    all_font_files.append(os.path.join(root, file_name))

        if not all_font_files:
            print("No fonts found in any subfolder.")
            return

        output_file = os.path.join(output_dir, "fonts_preview_all.png")
        print(f"\nGenerating combined preview for all fonts ({len(all_font_files)} fonts)")
        ...
        # Original behavior: one preview per subfolder
        for root, dirs, files in os.walk(root_folder):
            font_files = [
...
def main():
    """
    Main entry point: reads command-line arguments or interactive input,
    then calls generate_previews_for_subfolders with the appropriate options.
    """
    root_folder = None
    output_dir = None
    title_font_path = None
    ...
    if len(sys.argv) > 1:
        ...
        output = input("Output folder (Enter for same): ").strip()
        if output:
